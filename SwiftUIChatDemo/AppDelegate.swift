...
class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey : Any]? = nil) -> Bool {
        ...
        let channelNamer: ChatChannelNamer = {chnnel, currentUserId in
    ...
    private func connectUser() {
        ...
        chatClient.connectUser(userInfo: .init(id: "luke_skywalker", name: "luke_skywalker", imageURL: URL(string: "https://vignette.wikia.nocookie.net/starwars/images/2/20/LukeTLJ.jpg")!), token: token) {
            error in
            if let error = error {
                log.error("\(error)")
                return
